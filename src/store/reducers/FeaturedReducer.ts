...
export const FeaturedReducer = (state:IFeatuerd, action:TAction):IFeatuerd =>{
    switch(action.type) {
        case "ADD_FEATURED": return {...state, list: [...state['list'], action.payload]};
        case "REMOVE_FEATURED": return {...state, list: state['list'].filter(e=>e.id!=action.payload)}
