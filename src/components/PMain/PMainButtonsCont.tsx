...
const PMainButtonsCont = () => {
  ...
  return (
    <StyledContainerD $gap='10px'>
        <StyledButton $id={max - 2} $bgCol={Context.colorMain} $padding={[5, 5]} $active={checkActive(max-2)}/> 
        <StyledButton $id={max - 1} $bgCol={Context.colorMain} $padding={[5, 5]} $active={checkActive(max-1)}/> 
        <StyledButton $id={max} $bgCol={Context.colorMain} $padding={[5, 5]} $active={checkActive(max)}/> 
        <StyledButton $id={max + 1} $bgCol={Context.colorMain} $padding={[5, 5]} $active={checkActive(max+1)}/> 
        <StyledButton $id={max + 2} $bgCol={Context.colorMain} $padding={[5, 5]} $active={checkActive(max+2)}/> 
    </StyledContainerD>
  )
