...
const PArtSection: FC<IProps> = ({ info }) => {
  ...
  const changeFeatured = (act: 'add' | 'remove') => {
    ...
    if (act == 'add')
      Context.FeaturedDispatch({ type: 'ADD_FEATURED', payload: info });
    else
      Context.FeaturedDispatch({ type: 'REMOVE_FEATURED', payload: info.id });
  };
  ...
  return (
    ...
    >
      <StyledContainerD $aligni="center" $gap="20px">
        <h2 style={{ filter: 'drop-shadow(0px 0px 1px #ab68d8)' }}>
          {missingText(info!.title, 'title')}
        </h2>
        <FontAwesomeIcon
          icon={featured ? solidHeart : regularHeart}
          color={featured ? '#FF0000' : '#000'}
          size="2x"
          onClick={() => {
      ...
      <img
        style={{ width: '300px', boxShadow: '0px 0px 12px 2px #CCC' }}
      ...
      <h4>
        Created by{' '}
        <span style={{ color: Context.colorAdd }}>
          {missingText(info!.artist_title, 'Name')}
        </span>{' '}
        in {missingText(info!.place_of_origin, 'Place')}
        &nbsp;({missingText(info!.date_start!.toString(), 'Start')} -{' '}
        {missingText(info!.date_end!.toString(), 'Start')})
      </h4>
